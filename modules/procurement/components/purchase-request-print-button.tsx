...
type PurchaseRequestPrintApprovalStep = {
  stepNumber: number
...
export type PurchaseRequestPrintPayload = {
  ...
  discount: number
  grandTotal: number
  items: PurchaseRequestPrintItem[]
  approvalSteps: PurchaseRequestPrintApprovalStep[]
}

type PurchaseRequestPrintButtonProps = {
  payload: PurchaseRequestPrintPayload
  className?: string
}
...
const buildPurchaseRequestPrintHtml = (payload: PurchaseRequestPrintPayload): string => {
  const now = new Date()
  ...
  const requesterEmployeeNumber = escapeHtml(payload.requesterEmployeeNumber ?? "-")
  ...
  return `
    ...
    <html>
      <head>
        <meta charset="utf-8" />
        <title>Purchase Request ${escapeHtml(payload.requestNumber)}</title>
        <style>
      ...
      <body>
        ...
        <table class="meta">
          <tr>
            ...
            <td><span class="label">Employee #</span><span class="value">${requesterEmployeeNumber}</span></td>
          ...
          <tr>
            <td><span class="label">Series / Type</span><span class="value">${escapeHtml(`${payload.series}/${payload.requestType}`)}</span></td>
...
export const openPurchaseRequestPrintWindow = (payload: PurchaseRequestPrintPayload): void => {
  ...
  printWindow.document.write(buildPurchaseRequestPrintHtml(payload))
...
export function PurchaseRequestPrintButton({ payload, className }: PurchaseRequestPrintButtonProps) {
  const handlePrint = () => {
    openPurchaseRequestPrintWindow(payload)
  }
