...
import { updateLeaveRequestAction } from "@/modules/leave/actions/leave-request-actions"
import { updateLeaveRequestInputSchema } from "@/modules/leave/schemas/leave-request-actions-schema"

export const runtime = "nodejs"
export const dynamic = "force-dynamic"

const updatePayloadSchema = updateLeaveRequestInputSchema.omit({
  companyId: true,
  requestId: true,
})
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  if (companyRole !== "EMPLOYEE") {
    return mobileError("Only employees can update leave requests in mobile portal.", 403)
  }
  ...
  if (!parsed.success) {
    return mobileError(parsed.error.issues[0]?.message ?? "Invalid leave update payload.", 400)
  }
