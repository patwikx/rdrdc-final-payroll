...
type TrainingCertificationComplianceReportClientProps = {
  companyId: string
  companyName: string
...
const complianceToneByStatus: Record<
  TrainingCertificationComplianceRow["complianceStatus"],
...
export function TrainingCertificationComplianceReportClient({
  companyId,
  companyName,
  asOfDateValue,
  generatedAtLabel,
  ...
  summary,
  rows,
}: TrainingCertificationComplianceReportClientProps) {
  ...
  const exportHref = useMemo(() => {
    ...
    return `/${companyId}/reports/hr/training-certification-compliance/export?${params.toString()}`
  ...
  const printHref = useMemo(() => {
    ...
    if (filters.includeInactive) params.set("includeInactive", "true")
    if (filters.complianceScope !== "all") params.set("complianceScope", filters.complianceScope)
    return `/${companyId}/reports/hr/training-certification-compliance/print?${params.toString()}`
  }, [companyId, filters.complianceScope, filters.departmentId, filters.includeInactive])
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Employee training and credential compliance snapshot for expiry risk and missing-record audit checks.
        ...
        <div className="border-y border-border/70 bg-background">
          <div className="grid gap-2 border-b border-border/60 px-4 py-3 sm:px-6 lg:grid-cols-2 xl:grid-cols-5 lg:px-8">
            ...
            <MetricCard
              ...
              value={numberFormatter.format(summary.expiringSoonCount)}
              valueClassName="text-amber-700 dark:text-amber-300"
            />
            <MetricCard
              icon={IconAlertTriangle}
              label="Expired"
              value={numberFormatter.format(summary.expiredCount)}
              valueClassName="text-rose-700 dark:text-rose-300"
            />
            <MetricCard icon={IconFileAnalytics} label="No Records" value={numberFormatter.format(summary.noRecordCount)} />
          </div>
            ...
            <div className="flex flex-wrap items-end gap-2">
              ...
              <div className="w-full space-y-1 sm:w-[250px]">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    updateRoute({ complianceScope: value as ComplianceScope })
                  }}
                >
                  <SelectTrigger>
                    <SelectValue />
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="expired-only">Expired only</SelectItem>
              ...
              <Table className="w-full table-fixed border border-border/60 text-xs [&_th]:whitespace-normal [&_th]:border [&_th]:border-border/60 [&_td]:align-top [&_td]:whitespace-normal [&_td]:break-words [&_td]:border [&_td]:border-border/60">
                <TableHeader className="bg-muted/30">
                  <TableRow>
                    ...
                    <TableHead>Latest Training Date</TableHead>
                    <TableHead>Latest Credential Expiry</TableHead>
                    <TableHead className="text-right">Expired Credentials</TableHead>
                    <TableHead className="text-right">Expiring 30 Days</TableHead>
                ...
                <TableBody>
                  {pagedRows.length === 0 ? (
                    ...
                    pagedRows.map((row) => {
                      const tone = complianceToneByStatus[row.complianceStatus]
                      ...
                      return (
                        <TableRow key={row.employeeId}>
                          ...
                          <TableCell className="text-right">{numberFormatter.format(row.trainingCount)}</TableCell>
                          <TableCell className="text-right">{numberFormatter.format(row.certificationCount)}</TableCell>
                          <TableCell className="text-right">{numberFormatter.format(row.licenseCount)}</TableCell>
                          <TableCell>{row.latestTrainingDateValue ?? "-"}</TableCell>
                          <TableCell>{row.latestCredentialExpiryDateValue ?? "-"}</TableCell>
                          <TableCell className="text-right">{numberFormatter.format(row.expiredCredentialsCount)}</TableCell>
                          <TableCell className="text-right">{numberFormatter.format(row.expiringSoonCredentialsCount)}</TableCell>
                          <TableCell className="max-w-[320px] whitespace-normal break-words text-[11px] leading-4">
                            {row.complianceNotes}
                          </TableCell>
                        </TableRow>
                      )
