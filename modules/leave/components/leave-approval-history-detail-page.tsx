...
import type { EmployeePortalLeaveApprovalHistoryDetail } from "@/modules/leave/types/employee-portal-leave-types"

type LeaveApprovalHistoryDetailPageProps = {
  companyId: string
  requestId: string
  detail: EmployeePortalLeaveApprovalHistoryDetail
}
...
export function LeaveApprovalHistoryDetailPage({ companyId, requestId, detail }: LeaveApprovalHistoryDetailPageProps) {
  return (
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-xl font-semibold text-foreground sm:text-2xl">Leave Request History Detail</h1>
              <Badge variant="secondary" className="rounded-full border px-2 py-1 text-xs shadow-none">
                LEAVE
              </Badge>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button type="button" variant="outline" className="rounded-lg" asChild>
              <Link href={`/${companyId}/employee-portal/leave-approvals`}>Open Queue</Link>
            </Button>
          </div>
        </div>
      </div>
          ...
          <div className="lg:flex-1">
            <div className="grid grid-cols-1 gap-4">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label className="text-xs text-foreground">Leave Type</Label>
                  <Input value={detail.leaveTypeName} readOnly />
                </div>
              </div>

              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div className="space-y-2">
                  <Label className="text-xs text-foreground">Start Date</Label>
                  <Input value={detail.startDateLabel} readOnly />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs text-foreground">End Date</Label>
                  <Input value={detail.endDateLabel} readOnly />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs text-foreground">Duration</Label>
                  <Input value={`${detail.numberOfDays} day(s)`} readOnly />
                </div>
              </div>
