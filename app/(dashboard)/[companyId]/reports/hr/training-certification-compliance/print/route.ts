...
import { buildTrainingCertificationCompliancePrintHtml } from "@/modules/reports/hr/utils/training-certification-compliance-print-helpers"
...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    ...
    await createAuditLog({
      ...
      reason: "PRINT_TRAINING_CERTIFICATION_COMPLIANCE_REPORT",
    ...
    const html = buildTrainingCertificationCompliancePrintHtml({
      ...
      complianceScopeLabel: complianceScopeToLabel(viewModel.filters.complianceScope),
      rows: viewModel.rows,
    })
    ...
    return NextResponse.json(
      { error: "Unable to generate training and certification compliance print output." },
      { status: 500 }
    )
  }
}
