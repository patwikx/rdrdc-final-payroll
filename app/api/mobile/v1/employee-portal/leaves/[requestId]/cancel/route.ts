...
import { cancelLeaveRequestAction } from "@/modules/leave/actions/leave-request-actions"
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  if (companyRole !== "EMPLOYEE") {
    return mobileError("Only employees can cancel leave requests in mobile portal.", 403)
  }
  ...
  if (!parsed.success) {
    return mobileError(parsed.error.issues[0]?.message ?? "Invalid leave cancellation payload.", 400)
  }

  const { requestId } = await context.params
  const result = await cancelLeaveRequestAction({
