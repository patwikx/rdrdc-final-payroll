...
const getSourceRanksFromCompanyInputSchema = z
  .object({
...
type GetSourceRanksFromCompanyInput = z.infer<typeof getSourceRanksFromCompanyInputSchema>

type SourceRankListItem = {
  id: string
  code: string
  name: string
  level: number
...
type GetSourceRanksFromCompanyActionResult =
  | {
      ok: true
      ranks: SourceRankListItem[]
    }
  | {
      ok: false
      error: string
...
export async function getSourceRanksFromCompanyAction(
  input: GetSourceRanksFromCompanyInput
): Promise<GetSourceRanksFromCompanyActionResult> {
  const parsed = getSourceRanksFromCompanyInputSchema.safeParse(input)
  ...
  try {
    ...
    return { ok: false, error: `Failed to load source ranks: ${message}` }
  }
}
