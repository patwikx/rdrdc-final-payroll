...
import { buildGovernmentIdCompliancePrintHtml } from "@/modules/reports/hr/utils/government-id-compliance-print-helpers"
...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    ...
    await createAuditLog({
      ...
      reason: "PRINT_GOVERNMENT_ID_COMPLIANCE_REPORT",
    ...
    const html = buildGovernmentIdCompliancePrintHtml({
      ...
      complianceScopeLabel: governmentIdComplianceScopeToLabel(viewModel.filters.complianceScope),
      rows: viewModel.rows,
    })
    ...
    return NextResponse.json({ error: "Unable to generate government ID compliance print output." }, { status: 500 })
  }
}
