        ...
        <div className="relative flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Manage payroll pattern identity, statutory timing, and pay-period rows for each payroll year.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 border border-border/60 bg-background/90 p-2">
            ...
            <Button
              ...
              disabled={isTopControlsBusy || !canGenerateYear}
            >
              <IconCalendarEvent className="size-4" />
              {isGeneratingYear ? "Generating..." : "Generate Year"}
            </Button>
            <Button type="button" size="sm" className="h-8 px-2" onClick={handleSave} disabled={isTopControlsBusy}>
        ...
        <section className="border border-border/60 bg-background p-4">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-border/60 pb-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Define the primary payroll policy profile and statutory deduction timing.</p>
            </div>
            <div className="flex items-center gap-2">
              <Badge variant={form.isActive ? "default" : "outline"} className={form.isActive ? "bg-emerald-600 text-white" : ""}>
                {form.isActive ? "Active Policy" : "Inactive Policy"}
              </Badge>
              <Badge variant="outline">{yearValue}</Badge>
            </div>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field label="Periods Per Year" required>
              <Input
                ...
                onChange={(event) => updateField("periodsPerYear", Number(event.target.value))}
              />
            </Field>
            ...
            <div className="sm:col-span-2 lg:col-span-4">
              <div className="space-y-3 border border-border/60 bg-background/60 p-3">
                <div>
                  <p className="text-sm font-medium text-foreground">Statutory Deduction Schedule</p>
                  <p className="text-xs text-muted-foreground">Controls when each statutory deduction is applied within the pay cycle.</p>
                </div>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field label="Policy Active">
              <div className="flex h-9 items-center justify-between border border-border/70 bg-background px-2.5">
                <span className="text-xs text-muted-foreground">Enable this policy</span>
                <Switch checked={form.isActive} onCheckedChange={(checked) => updateField("isActive", checked)} />
              </div>
            </Field>
            <div className="sm:col-span-2 lg:col-span-4">
              <Field label="Description">
                <Textarea className="h-20" value={form.description ?? ""} onChange={(event) => updateField("description", event.target.value)} />
              </Field>
            </div>
          </div>
        </section>
    ...
    <section className="border border-border/60 bg-background p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button type="button" size="sm" className="h-8 px-2" onClick={onSaveRows} disabled={isBusy || isYearLocked}>
            <IconCheck className="size-4" />
            {isSavingRows ? "Saving..." : "Save Period Rows"}
          </Button>
        </div>
      </div>
      <div className="mt-3 overflow-x-auto border border-border/60">
