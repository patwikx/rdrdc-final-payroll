              ...
              <div className="flex flex-col gap-2 border-b border-border/60 bg-muted/20 px-3 py-3 sm:flex-row sm:items-center">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    placeholder="Search request #, leave type, reason"
                    className="pl-9"
                  />
                </div>
                <Select
              ...
              {filteredRequests.length > 0 ? (
                ...
                return (
                  <div key={request.id} className={cn("group border-b border-border/60 last:border-b-0 cursor-pointer transition-colors", isExpanded && "bg-primary/10")}>
                    <div className="hidden grid-cols-12 items-center gap-3 px-3 py-4 md:grid" onClick={() => setExpandedRequestId(isExpanded ? null : request.id)}>
                      ...
                      <div className="col-span-2 text-sm text-foreground leading-tight"><div>{request.startDate}</div><div className="text-muted-foreground text-xs">to {request.endDate}</div></div>
                      ...
                      <div className="col-span-2"><p className="line-clamp-2 text-xs text-foreground">{request.reason || "-"}</p></div>
                      <div className="col-span-2">
                      ...
                      <div className="col-span-1 flex justify-end" onClick={(e) => e.stopPropagation()}>
                        {request.statusCode === "PENDING" ? (
                          ...
                          <div className="text-right text-xs text-muted-foreground">
                            {request.rejectionReason ? <p className="text-destructive">Reason: {request.rejectionReason}</p> : null}
                          </div>
                        )}
                      </div>
                    </div>
