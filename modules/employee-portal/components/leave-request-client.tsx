...
export function LeaveRequestClient({ companyId, leaveTypes, leaveBalances, requests }: LeaveRequestClientProps) {
  ...
  const openEditDialog = (request: EmployeePortalLeaveRequestRow) => {
    setEditingRequestId(request.id)
    setLeaveTypeId(request.leaveTypeId)
    setStartDate(parseDateInputValue(request.startDateInput))
    setEndDate(parseDateInputValue(request.endDateInput))
    setIsHalfDay(request.isHalfDay)
    setHalfDayPeriod(request.isHalfDay && (request.halfDayPeriod === "AM" || request.halfDayPeriod === "PM") ? request.halfDayPeriod : "")
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      const result = editingRequestId
        ? await updateLeaveRequestAction({
            companyId,
