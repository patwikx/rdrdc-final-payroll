...
import { PurchaseRequestPrintButton } from "@/modules/procurement/components/purchase-request-print-button"
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-semibold text-foreground sm:text-2xl">Purchase Request Details</h1>
              <Badge
                variant={statusVariant(request.status)}
                className="rounded-full border px-2 py-1 text-xs shadow-none"
              >
                {requestStatusLabel}
              </Badge>
            </div>
          ...
          <div className="flex items-center gap-2">
            <PurchaseRequestPrintButton
              payload={{
                ...
                rejectedAtLabel: request.rejectedAtLabel,
                cancelledAtLabel: request.cancelledAtLabel,
      ...
      <div className="space-y-5 p-4 sm:p-5">
        ...
        <div className="border-t border-border/60 pt-4 sm:pt-5">
          ...
          <div className="grid grid-cols-1 gap-3 text-xs text-muted-foreground md:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              <p className="font-medium text-foreground">Requester #</p>
              <p>{request.requesterEmployeeNumber}</p>
            </div>
            <div>
              <p className="font-medium text-foreground">Branch</p>
              <p>{request.requesterBranchName ?? "-"}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p>{request.approvedAtLabel ?? "-"}</p>
            </div>
            <div>
              <p className="font-medium text-foreground">Rejected At</p>
              <p>{request.rejectedAtLabel ?? "-"}</p>
            </div>
            <div>
            ...
            <Table className="min-w-[980px] text-xs">
              <TableHeader>
                <TableRow className="border-b border-border/60 bg-muted/30 hover:bg-muted/30">
                  ...
                  <TableHead className="h-9 w-[130px] px-3 text-right text-[11px] uppercase tracking-wide">
                    Unit Price
                  </TableHead>
                  <TableHead className="h-9 w-[130px] px-3 text-right text-[11px] uppercase tracking-wide">
                    Line Total
        ...
        <div className="border-t border-border/60 pt-4 sm:pt-5">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="px-1 py-2">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Finalization Lead Time</p>
              <p className="mt-1 text-lg font-semibold text-foreground">{request.finalizationLeadTimeLabel ?? "-"}</p>
              <p className="text-[11px] text-muted-foreground">Submitted to final decision</p>
            </div>

            <div className="px-1 py-2">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Pending Age</p>
              <p className="mt-1 text-lg font-semibold text-foreground">{request.pendingAgeLabel ?? "-"}</p>
              <p className="text-[11px] text-muted-foreground">Elapsed while pending approval</p>
            </div>

            <div className="px-1 py-2">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Draft to Submit</p>
              <p className="mt-1 text-lg font-semibold text-foreground">{request.draftToSubmitLeadTimeLabel ?? "-"}</p>
              <p className="text-[11px] text-muted-foreground">Created to first submission</p>
            </div>

            <div className="px-1 py-2">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Total Lifecycle</p>
              <p className="mt-1 text-lg font-semibold text-foreground">{request.totalLifecycleLeadTimeLabel ?? "-"}</p>
              <p className="text-[11px] text-muted-foreground">Created to final decision</p>
            </div>
          </div>
        </div>
      </div>
