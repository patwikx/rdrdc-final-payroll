...
export function PurchaseRequestClient({
  companyId,
  requests,
  canCreateRequest,
}: PurchaseRequestClientProps) {
      ...
      <div className="flex flex-col justify-between gap-3 border-b border-border/60 bg-muted/30 px-4 py-4 sm:px-6 md:flex-row md:items-end">
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-4">
            <h1 className="text-xl font-semibold text-foreground sm:text-2xl">Purchase Requests</h1>
        ...
        {canCreateRequest ? (
          <Button className="rounded-lg bg-primary hover:bg-primary/90" asChild>
            <Link href={`/${companyId}/employee-portal/purchase-requests/new`}>
        ...
        <div>
          ...
          {requests.length === 0 ? (
            <div className="space-y-3 rounded-2xl border border-dashed border-border/60 bg-muted/30 py-14 text-center">
              <p className="text-sm font-semibold text-foreground">No Purchase Requests Yet</p>
              <p className="text-sm text-muted-foreground">
                {canCreateRequest
                  ? "Create your first purchase request draft to get started."
            ...
            <div className="lg:overflow-hidden lg:rounded-2xl lg:border lg:border-border/60 lg:bg-card">
              <div className="grid grid-cols-3 gap-2 border-b border-border/60 bg-muted/20 px-3 py-3 sm:flex sm:flex-wrap sm:items-center">
                <div className="relative col-span-3 sm:w-[360px] sm:flex-none">
                  ...
                  <Input
                    ...
                    placeholder="Search request #, requester, type, notes"
                ...
                <Button
                  type="button"
                  variant="outline"
                  className="col-span-1 w-full rounded-lg sm:w-auto"
                  onClick={() => {
                    setLogSearch("")
                    setLogStatus("ALL")
                    setCurrentPage(1)
                ...
                <div className="space-y-2 lg:hidden">
                  {paginatedRows.map((request) => {
                    ...
                    const requestHref = canEdit
                      ? `/${companyId}/employee-portal/purchase-requests/${request.id}/edit`
                      : `/${companyId}/employee-portal/purchase-requests/${request.id}`
                        ...
                        >
                          ...
                          <div className="mt-3 rounded-lg border border-border/60 bg-muted/20 p-2.5">
                            ...
                            <p className="mt-1 text-sm font-semibold text-foreground">
                              {request.series}/{request.requestType}
                            </p>
                        ...
                        >
                          ...
                          {canEdit ? (
                            <Button type="button" variant="outline" size="sm" className="h-8 rounded-lg text-xs" asChild>
                              <Link href={`/${companyId}/employee-portal/purchase-requests/${request.id}/edit`}>
                          ...
                          {canSubmit ? (
                            <AlertDialog>
                              <AlertDialogTrigger asChild>
                                <Button type="button" size="sm" className="h-8 rounded-lg text-xs" disabled={isPending}>
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent className="rounded-xl border-border/60 shadow-none">
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    This will mark the purchase request as cancelled.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                            ...
                            >
                              <div className="space-y-2 border-t border-border/60 bg-muted/30 px-3 py-3 text-xs">
                                ...
                                {request.finalDecisionRemarks ? (
                                  <div>
                                    <p className="text-[11px] text-muted-foreground">Decision Remarks</p>
                                    <p className="text-foreground">{request.finalDecisionRemarks}</p>
                                  </div>
                                ) : null}
                                {request.cancellationReason ? (
                                  <div>
                                    <p className="text-[11px] text-muted-foreground">Cancellation Reason</p>
                                    <p className="text-foreground">{request.cancellationReason}</p>
                                  </div>
                                ) : null}
                              </div>
                            </motion.div>
              ...
              <div className="hidden grid-cols-12 items-center gap-3 border-b border-border/60 bg-muted/30 px-3 py-2 lg:grid">
                <p className="col-span-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                  Request #
                </p>
                ...
                <p className="col-span-1 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                  Amount
                </p>
                <p className="col-span-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Status</p>
                <p className="col-span-1 text-right text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                  Action
                </p>
              </div>
                              ...
                              <div className="flex items-center gap-1">
                                {canEdit ? (
                                  <Tooltip>
                                    ...
                                    <TooltipContent side="top" sideOffset={6}>Edit Request</TooltipContent>
                                  </Tooltip>
                                ) : null}
                                ...
                                {canCancel ? (
                                  <AlertDialog>
                                    <Tooltip>
                                      ...
                                      <TooltipContent side="top" sideOffset={6}>Cancel Request</TooltipContent>
                                    </Tooltip>
                                    <AlertDialogContent className="rounded-xl border-border/60 shadow-none">
                                      <AlertDialogHeader>
                                        <AlertDialogTitle className="text-base font-semibold">Cancel Request</AlertDialogTitle>
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      type="button"
                                      size="sm"
                                      className="h-8 rounded-lg px-2 text-xs bg-primary hover:bg-primary/90"
                                      asChild
                                    >
                                      <Link href={requestHref}>
                            ...
                            <div className="space-y-3 border-t border-border/60 bg-muted/30 px-4 py-3">
                              ...
                              <div className="overflow-hidden border border-border/60 bg-card">
                                <div className="grid grid-cols-12 items-center gap-2 border-b border-border/60 bg-muted/30 px-2 py-2">
                                  <p className="col-span-1 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                                    Line
                                  </p>
                                  <p className="col-span-2 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                                    Code
                                  </p>
                                  <p className="col-span-3 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                                    Description
                                  </p>
                                  <p className="col-span-1 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                                    UOM
                                ...
                                {request.items.map((item) => (
                                  <div
                                    key={item.id}
                                    className="grid grid-cols-12 items-start gap-2 border-b border-border/60 px-2 py-2 text-xs last:border-b-0"
                                  >
