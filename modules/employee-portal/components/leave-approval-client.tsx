...
export function LeaveApprovalClient({
  ...
  const submit = () => {
    ...
    if (!optimisticRow) {
      toast.error("The selected leave request is no longer in the queue.")
      setOpen(false)
      setSelectedId(null)
    ...
    startTransition(async () => {
      const response = isHR
        ? actionType === "approve"
          ? await approveLeaveByHrAction({ companyId: requestCompanyId, requestId: resolvedSelectedId, remarks: resolvedRemarks })
          : await rejectLeaveByHrAction({ companyId: requestCompanyId, requestId: resolvedSelectedId, remarks: resolvedRemarks })
        : actionType === "approve"
          ? await approveLeaveBySupervisorAction({ companyId: requestCompanyId, requestId: resolvedSelectedId, remarks: resolvedRemarks })
          : await rejectLeaveBySupervisorAction({ companyId: requestCompanyId, requestId: resolvedSelectedId, remarks: resolvedRemarks })
	              ...
	              <div className="rounded-xl border border-border/60 bg-muted/25 p-2.5">
	                <div className="space-y-2">
	                  <div className="flex items-start justify-between gap-2">
	                    <p className="min-w-0 flex-1 truncate text-xs text-foreground/70">{selected?.requestNumber ?? "-"}</p>
	                    {selected ? (
